#!/usr/bin/env python3
"""Mutation tests for the fixed-polygon-v8 R<=9 assembler wrapper."""
...
SOURCE = HERE / "assemble_one_band_236_fixed_polygon_v8_r09.py"
...
M = load("fixed_polygon_v8_r09_assembler_test", SOURCE)
T8 = load("fixed_polygon_v8_r09_conversion", V8_TEST)
T7 = load("fixed_polygon_v8_r09_v7_fixture", V7_TEST)
T6 = load("fixed_polygon_v8_r09_v6_fixture", V6_TEST)
...
def fixture(count):
    v7 = T7.to_v7(M.V8.V7, T6.synthetic_r0(M.V8.V7.V6))
    raw = T8.to_v8(M.V8, v7)
    raw["common_r"] = count
...
class FixedPolygonV8R09AssemblerTest(unittest.TestCase):
    def test_real_v8_audit_parser_and_r9_selection(self):
        raw = fixture(9)
        data = canonical(raw)
        full = M.parse_b_v8(Path("common_r_09.json"), data, 9)
    ...
    def test_v8_identity_and_count_mutations_fail(self):
        raw = fixture(4)
        mutant = copy.deepcopy(raw)
        mutant["algorithm"][
            "polygon_runtime_module_patched_after_pinned_load"] = False
        with self.assertRaises(ValueError):
            M.parse_b_v8(Path("common_r_04.json"), canonical(mutant), 4)
        with self.assertRaises((ValueError, ArithmeticError)):
            M.parse_b_v8(Path("common_r_03.json"), canonical(raw), 3)
    ...
    def test_flat_source_closure_and_main_restores_parser(self):
        ...
        with tempfile.TemporaryDirectory(prefix="v8-r09-main-") as root_text:
            root = Path(root_text)
