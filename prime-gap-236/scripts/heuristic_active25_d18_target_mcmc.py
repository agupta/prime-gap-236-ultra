...
def cap_indicator(points, delta, schedule):
    ...
    sums = np.sum(np.where(large, points, 0), axis=1,
                  dtype=np.longdouble)
    result = counts == 0
    ratios = np.full(len(points), np.inf, dtype=np.longdouble)
    ratios[counts == 0] = 0
    for count in range(1, len(schedule) + 1):
        selected = counts == count
        result |= (selected & (sums <= schedule[count - 1]))
        ratios[selected] = sums[selected] / schedule[count - 1]
    return result, counts, ratios
...
def run_mcmc(polynomial, midpoint, *, seed, chains, burn, steps, thin):
    ...
    ratio_rows = [[] for _ in range(chains)]
    ...
    for iteration in range(total_steps):
        ...
        if iteration >= burn and (iteration - burn) % thin == 0:
            ...
            sums = np.sum(points, axis=1, dtype=np.longdouble)
            cap, counts, ratios = cap_indicator(points, delta, schedule)
            ...
            for chain in range(chains):
                ...
                ratio_rows[chain].append(float(ratios[chain]))
    ...
    return {
        ...
        "cap_ratios": np.asarray(ratio_rows, dtype=float),
...
def main():
    ...
    calibration_pass = (mid["rhat"] <= 1.05 and mid["ess"] >= 200 and
                        abs(calibration_z) <= 5)
    cap_hits = int(np.sum(samples["cap"]))
    cap_misses = int(samples["cap"].size - cap_hits)
    cap_diagnostic_pass = (cap["rhat"] <= 1.05 and cap["ess"] >= 200 and
                           cap_hits >= 100 and cap_misses >= 100)
    ...
    result = {
        ...
        "cap_event_counts": {"hits": cap_hits, "misses": cap_misses,
                             "minimum_required_each": 100},
        "cap_ratio_quantiles": {
            str(q): float(np.quantile(samples["cap_ratios"], q))
            for q in (0, .01, .1, .5, .9, .99, 1)
        },
        ...
        "format": "active25-d18-target-density-mcmc-v2",
