#!/usr/bin/env python3
"""Strict post-run assembler/consumer for the active25 D16 staged pencil.
...
HERE = FILE.parent
REPO = FILE.parents[2]
STAGED = HERE / "frontier_active25_inner_d16_staged_v3.py"
STAGED_TEST = HERE / "test_frontier_active25_inner_d16_staged_v3.py"
PINNED = {
...
_START = snapshots()
_SPEC = importlib.util.spec_from_file_location("active25_staged_v3_core", STAGED)
...
def load_completed_manifest(record_dir, expected_sha256):
    ...
    try:
        ...
        staged.strict_manifest(manifest, handle, expected_sha256)
        stages = []
        bindings = []
        for r, row in enumerate(manifest["stages"]):
            ...
            stages.append(staged.parse_stage_bytes(stage_snap["data"], r))
            bindings.append(stage_snap)
        return handle, snap, manifest, stages, bindings
    except Exception:
...
def build_result(record_dir, expected_manifest_sha256):
    ...
    handle, manifest_snap, manifest, stages, bindings = \
    ...
    try:
        ...
        staged.strict_manifest(manifest, handle, expected_manifest_sha256)
...
def strict_result(value, expected_manifest_sha256):
    ...
    if (value["assembler_sha256"] != sha256(FILE) or
            ...
            value["format"] !=
            "frontier-active25-inner-D16-exact-pencil-v3" or
...
def _rebind_manifest_handle(handle, expected_manifest_sha256):
    ...
    staged.strict_manifest(value, handle, expected_manifest_sha256)
    return snap
