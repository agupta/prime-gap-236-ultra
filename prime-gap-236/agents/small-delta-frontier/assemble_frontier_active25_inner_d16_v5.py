#!/usr/bin/env python3
"""Strict v5 post-run assembler/consumer for the active25 D16 pencil.

The caller must provide the externally recorded SHA-256 of the completed
26-shard manifest and the externally recorded ledger SHA/device/inode.  Every
shard byte/inode and all frozen arithmetic inputs are rebound before exact
assembly.  Numerical eigendiscovery is used only to select a rational vector;
its particular I and 48J forms are exact Fractions.  This consumer does not
independently recompute shard arithmetic and therefore is never theorem-ready.
"""
...
HERE = FILE.parent
REPO = FILE.parents[2]
STAGED = HERE / "frontier_active25_inner_d16_staged_v5.py"
STAGED_TEST = HERE / "test_frontier_active25_inner_d16_staged_v5.py"
PINNED = {
    STAGED: "8bb0d5088e419d196b4aca732ec384804bf2918554cf001f372a3127e7f1f775",
    STAGED_TEST: "28b5b8d182fc0836a0bf905af4d7f9b907b403e35a9e61cae779428fb7f899bf",
}
...
_START = snapshots()
_SPEC = importlib.util.spec_from_file_location("active25_staged_v5_core", STAGED)
...
def require_self_sha256(value):
    strict_sha(value, "expected assembler self SHA")
    if value != sha256(FILE):
        raise RuntimeError("externally supplied assembler SHA does not match")
    return value


def expected_ledger_binding(sha256, device, inode):
    staged.strict_sha(sha256, "expected ledger SHA")
    ...
    return {"leaf": staged.LEDGER_LEAF, "sha256": sha256,
            "device": device, "inode": inode}


def load_completed_manifest(record_dir, expected_sha256, expected_ledger):
    ...
    if (type(expected_ledger) is not dict or set(expected_ledger) != {
        ...
        raise ValueError("external ledger binding is missing")
...
def assemble_exact_matrices(inner_i, inner_b, raw_cross, active, masses,
                            shell_b, *, k=48):
    if (type(k) is not int or k != 48 or
            type(active) is not list or active != list(range(len(active))) or
...
def build_result(record_dir, expected_manifest_sha256, expected_ledger):
...
def strict_result(value, expected_manifest_sha256, expected_ledger):
    ...
    if (value["assembler_sha256"] != sha256(FILE) or
            ...
            value["format"] !=
            "frontier-active25-inner-D16-exact-pencil-v5" or
            ...
            {key: ledger_binding[key] for key in
             ("sha256", "device", "inode")} !=
            {key: expected_ledger[key] for key in
             ("sha256", "device", "inode")} or
...
def read_external_file(path, expected_sha256):
    ...
    descriptor = None
    try:
        descriptor = os.open(staged._safe_leaf(target.name), flags,
                             dir_fd=parent["descriptor"])
        before = os.fstat(descriptor)
        if not stat.S_ISREG(before.st_mode) or before.st_nlink != 1:
            raise ValueError("assembled result is not singly linked regular data")
        chunks = []
        while True:
        ...
        staged.validate_record_dir(parent)
    finally:
...
def _rebind_manifest_handle(handle, expected_manifest_sha256,
    ...
    ledger_snap = staged.read_leaf(handle, staged.LEDGER_LEAF)
    if staged.ledger_binding(ledger_snap) != expected_ledger:
        raise RuntimeError("external ledger binding changed before publication")
...
def main():
    ...
    require_self_sha256(args.expected_self_sha256)
    expected_ledger = expected_ledger_binding(
    ...
    strict_result(value, args.expected_manifest_sha256, expected_ledger)
    rebuilt = build_result(args.record_dir,
                           args.expected_manifest_sha256, expected_ledger)
