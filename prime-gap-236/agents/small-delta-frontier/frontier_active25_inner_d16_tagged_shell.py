...
large-count stratum of the outer shell.  Thus its dimension is 27.  The shell
I block is diagonal, the shell J block is tridiagonal, and all 26 inner/shell
...
def validate_analytic():
    ...
    if (raw.get("status") != "AUDIT PASS" or
            raw.get("schedule_id") != "nonuniform-outer-active25-tail-v4" or
            ...
            parameters.get("outer_final_plateau") != str(SCHEDULE[-1]) or
            parameters.get("outer_active") != list(range(26)) or
...
def grouped_weighted_cross(named, pair_catalog, pair_weights, common_eta, *,
    ...
    for r in selected:
        ...
        if (selected_h is not None and
                (type(selected_h) is not int or
                 not 0 <= selected_h <= max_h)):
            raise ValueError("selected h is outside face list")
        for h in h_values:
            outer = common_eta - (r + h) * delta
...
def shell_i_and_j():
    ...
    expected_active = validate_analytic()["parameters"]["outer_active"]
...
def preflight():
    ...
    result = {
        ...
        "dimension": 1 + len(validate_analytic()["parameters"]["outer_active"]),
...
def build_cost_probe(common_r, selected_h, *, mode="direct-full-grouped"):
    started = time.monotonic()
    self_start = sha256(FILE)
    pins = require_pins()
    validate_analytic()
    named, catalog, amplitudes, _, _ = production_cross_inputs()
    if mode == "ungrouped-four-branch-oracle":
        raw_tables, counts, faces = tagged_cross_catalog(
            named, catalog, ETA2, common_strata=(common_r,),
            selected_h=selected_h, integrate=True, progress=True)
        combined = combine_radial_cross(raw_tables, amplitudes)
        geometric_groups = None
        nonzero_groups = None
    elif mode in ("four-branch-grouped", "direct-full-grouped"):
        combined, counts, geometric_groups, nonzero_groups, faces = \
            grouped_weighted_cross(
                ...
                direct_full_left=(("R", "V") if
                                  mode == "direct-full-grouped" else ()),
                progress=True)
        raw_tables = None
    else:
        raise ValueError("unknown cost-probe mode")
    ...
    if raw_tables is not None:
        result["raw_cross_by_pair_and_target_R"] = {
            tag: [str(x) for x in values]
            for tag, values in raw_tables.items()}
    return result
...
def main():
    ...
    parser.add_argument("--ungrouped-oracle", action="store_true")
    ...
    if args.preflight_only:
        ...
        if args.ungrouped_oracle:
            parser.error("preflight does not accept ungrouped oracle mode")
    ...
    if args.literal_full_branches and args.ungrouped_oracle:
        parser.error("choose at most one oracle mode")
    mode = ("ungrouped-four-branch-oracle" if args.ungrouped_oracle else
            "four-branch-grouped" if args.literal_full_branches else
            "direct-full-grouped")
    result = build_cost_probe(args.cost_probe_r, args.cost_probe_h, mode=mode)
