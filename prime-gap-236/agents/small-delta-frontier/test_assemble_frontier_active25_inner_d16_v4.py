...
SOURCE = FILE.with_name("assemble_frontier_active25_inner_d16_v4.py")
SPEC = importlib.util.spec_from_file_location("assemble_active25_v4", SOURCE)
...
class AssemblerTests(unittest.TestCase):
    ...
    def test_o_excl_output_and_record_alias_rejection(self):
        with tempfile.TemporaryDirectory() as parent:
            ...
            try:
                M._rebind_manifest_handle = lambda handle, expected: None
                snap = M.publish_output(target, value, record, manifest_sha)
                self.assertEqual(
                    snap["sha256"],
                    M.sha256_bytes(M.staged.canonical_json(value)))
                with self.assertRaises(FileExistsError):
                    M.publish_output(target, value, record, manifest_sha)
                with self.assertRaises(ValueError):
