#!/usr/bin/env python3
"""Build a production-disabled D12/v6.2 identity-and-baseline gate."""
...
import importance_d12_target_v62 as target
...
SOURCE_PATHS = (
    ...
    "agents/structural-basis/code/importance_d12_target_v62.py",
    "agents/structural-basis/code/build_importance_d12_target_v62_gate.py",
    "agents/structural-basis/tests/test_importance_d12_target_v62.py",
    "agents/structural-basis/IMPORTANCE-D12-TARGET-V62-SPEC.md",
    *target.V62_AUDIT_ARTIFACT_HASHES,
)
...
def build_gate(expected_self_sha256, d4_gate_path):
    ...
    target.validate_v62_audit_artifacts()
    d4_bound = target.d4.load_and_validate_gate(d4_gate_path)
    d4_gate = d4_bound["gate"]
    if (d4_bound["sha256"] != target.d4.V61_GATE_SHA256 and
            d4_bound["sha256"] !=
            target.core.sha256_file(d4_gate_path)):
        # The second equality is the ordinary path.  The first disjunct is
        # intentionally never a success for the successor gate and prevents
        # confusing the predecessor hash with the current gate hash.
        raise ValueError("D4 v6.2 gate snapshot changed")
    if (d4_gate["status"] !=
            "frozen-d4-exact-whitened-calibration-prelaunch-v6.2" or
            d4_gate["production_launch_authorized"] is not False):
        raise ValueError("D4 v6.2 gate is not the frozen prelaunch gate")
    ...
    for relative, expected in target.V62_AUDIT_ARTIFACT_HASHES.items():
        if source_hashes.get(relative) != expected:
            raise ArithmeticError("target gate does not bind v6.2 PASS")
    ...
    return {
        "status": "frozen-d12-transformed-target-identity-prelaunch-v6.2",
        "rigorous": False,
        "screen_launch_authorized": False,
        "identity_dry_run_only": True,
        "d4_v62_gate_path": D4_GATE_RELATIVE,
        "d4_v62_gate_sha256": d4_gate_sha,
        "requires_independently_audited_d4_v62_calibration_pass": True,
        "transform_sha256": target.d4.v61.v6.TRANSFORM_SHA256,
        "base_target": {
        ...
        "schedule": target.d4.v61.v6.v5.expected_schedule(),
        "unchanged_statistical_thresholds":
            target.d4.v61.v6.v5.expected_thresholds(),
        "data_split": {
        ...
        "continuation_rule": target.d4.v61.v6.expected_continuation_rule(),
        "next_stage": {
...
def main():
    ...
    target.d4.v61.v6._patch_v5_runtime()
    target.d4.v61.v6.v5.REQUIRED_SOURCE_PATHS = tuple(gate["source_hashes"])
    target.d4.v61.v6.v5.REQUIRED_DATA_PATHS = tuple(gate["data_hashes"])
    digest = target.d4.v61.v6.v5.write_new_result(args.output, gate, gate)
    print(digest)
