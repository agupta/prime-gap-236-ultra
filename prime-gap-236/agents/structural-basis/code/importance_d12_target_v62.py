#!/usr/bin/env python3
"""V6.2-bound unmultiplied D12 multiplier target, identity dry-run only.

The numerical target is the byte-pinned 272-label D12 polynomial, not the
negative D4-quadratic transfer.  This module performs exact source/transform
identity checks and Decimal normalizer checks only.  It runs no Markov chain,
selects no candidate, and computes no new sieve quotient.
"""
...
import importance_d4_calibration_v62 as d4
...
# Filled only after a separately frozen v6.2 AUDIT PASS.  The target builder
# rejects an empty mapping, so this adapter cannot turn a merely self-tested
# calibration package into a D12 launch gate.
V62_AUDIT_ARTIFACT_HASHES = {}
...
def validate_v62_audit_artifacts():
    if not V62_AUDIT_ARTIFACT_HASHES:
        raise ValueError("independent v6.2 PASS artifacts are not pinned")
    for relative, expected in V62_AUDIT_ARTIFACT_HASHES.items():
        if core.sha256_file(REPO_ROOT / relative) != expected:
            raise ValueError(f"v6.2 audit artifact changed: {relative}")
    return True
...
def exact_identity_package(repo_root=REPO_ROOT):
    ...
    if (adapter.whitening_transform_sha256 != d4.v61.v6.TRANSFORM_SHA256 or
            adapter.dimension != 96 or adapter.k != 48 or
            tuple(adapter.strata) != tuple(range(16))):
        raise ValueError("D12 adapter transform/dimension differs from v6.2")
...
# Re-export only the source-bound target pieces that a later separately
# audited screen driver will consume.
