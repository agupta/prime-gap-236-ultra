#!/usr/bin/env python3
"""Exact D14-grid38 cross shard with global integer product collection.

This v5 wrapper retains the audited maximum-shift-pruned radial transform.
For each surviving shift it collects the entire affine--radial product by its
final aggregate monomial over integers, then performs one exact polygon-
moment multiplication per surviving monomial.
"""
...
V5_PATH = FILE.with_name("collected_integer_scalar.py")
V3_TEST_PATH = FILE.with_name("test_pruned_integer_radial.py")
V5_TEST_PATH = FILE.with_name("test_collected_integer_scalar.py")
...
LOCAL_PINNED = {
    ...
    V5_PATH: "aef0a183d71b9c41b5373806d03481b94cb4e61a1ff3561888b8c31f94e8c890",
    V3_TEST_PATH: "17b5eac692f859728d502e90a52b2c9c5ce03ef45e7966ce9104d8878910adfd",
    V5_TEST_PATH: "281ff70246041cc0d2ed948c41187b406a26b9d7ba9b94a1f4b502dab63d31d4",
...
def main():
    ...
    if sha256_bytes(self_data) != args.expected_self_sha256:
        raise RuntimeError("externally pinned collected-v5 runner SHA mismatch")
    ...
    v2 = load("d14_grid38_collected_v5_v2", V2_PATH,
    ...
    base = load("d14_grid38_collected_v5_base", v2.BASE_PATH,
                v2_snapshots[v2.BASE_PATH])
    fast_v2 = load("d14_grid38_collected_v5_fast_v2", v2.FAST_PATH,
                   v2_snapshots[v2.FAST_PATH])
    pruned_v3 = load("d14_grid38_collected_v5_pruned_v3", V3_PATH,
                     local_snapshots[V3_PATH])
    collected_v5 = load("d14_grid38_collected_v5_algorithm", V5_PATH,
                        local_snapshots[V5_PATH])
    pruned_v3.FAST_V2 = fast_v2
    collected_v5.FAST_V2 = fast_v2
    collected_v5.PRUNED_V3 = pruned_v3
    publish_source = load(
        "d14_grid38_collected_v5_publish", PUBLISH_SOURCE_PATH,
    ...
    result = v2.build(
        args.common_r, v2_snapshots, dependency_snapshots, base, collected_v5)
    result["format"] = (
        "D14-grid38-scaled-cutoff-cross-common-r-collected-v5")
    result["status"] = "EXACT COLLECTED COMMON-r CROSS SHARD PASS"
    result["algorithm"] = {
    ...
    if (FILE.read_bytes() != self_data or
        ...
        raise RuntimeError("collected-v5 source closure changed during computation")
